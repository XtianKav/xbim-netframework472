...
namespace libal.Services
{
    public class IfcStoreGenerator
    {
        ...
        {
            return await GetIfcStoreForIfcFileAsync();
        }

        private async Task<IfcStore> GetIfcStoreForIfcFileAsync()
        {
            var ifcSchemaVersion = await GetSchemaVersionAsync();
            var store = IfcStore.Open(_ifcStream, Xbim.IO.StorageType.Ifc, ifcSchemaVersion, XbimModelType.MemoryModel);
            return store;
        }

        private async Task<Xbim.Common.Step21.XbimSchemaVersion> GetSchemaVersionAsync()
        {
            Xbim.Common.Step21.XbimSchemaVersion schemaVersion = await GetSchemaVersionFromIfcFileStreamAsync(_ifcStream);
